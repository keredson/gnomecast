import contextlib, os, signal, socket, subprocess, sys, tempfile, threading, time, traceback, urllib
...
class Transcoder(object):
  ...
  def __init__(self, cast, fn, force_audio=False, force_video=False, trans_dir=None):
    self.cast = cast
    self.source_fn = fn
    self.p = None
    ...
    if self.transcode:
      self.done = False
      self.trans_dir = trans_dir or tempfile.mkdtemp(prefix='gnomecast_')
      if not os.path.isdir(self.trans_dir):
  ...
  def do_transcode(self):
    ...
    for s in iter(self.p.stdout.readline, ""):
        s = s.strip()
        print('transcode:', s)
        m = re.search(r"/(output_\d+.ts)' for writing", s)
        if m:
...
class Gnomecast(object):
  ...
  def start_server(self):
    ...
    def video(id, fn):
      if fn.endswith('.ts') or fn.endswith('.m3u8'):
        if fn.endswith('.m3u8'):
          with open(os.path.join(self.transcoder.trans_dir, fn)) as f:
            print(repr(f.read()))
  ...
  def build_gui(self):
    ...
    self.files_store = Gtk.ListStore(str, str, int, str, str, int, str, object) # name, path, duration, duration_str, thumbnail_fn, not_used, status_icon, transcoder
  ...
  def force_transcode(self, audio=True, video=True):
    for row in self.files_store:
      ...
      transcoder = row[7]
      transcoder.destroy()
      self.transcoder = Transcoder(self.cast, self.fn, force_audio=audio, force_video=video)
    ...
    if self.cast and self.fn:
      ...
      for row in self.files_store:
        ...
        if not transcoder or self.cast != transcoder.cast or self.fn != transcoder.source_fn:
          ...
          self.transcoder = Transcoder(self.cast, self.fn)
          row[7] = self.transcoder
      if self.autoplay:
        self.autoplay = False
