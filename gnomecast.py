...
class FileMetadata(object):
    ...
    def __init__(self, fn, callback=None, _ffmpeg_output=None):
        ...
        def parse():
            ...
            thumbnail_fn = tempfile.mkstemp(
                suffix='.jpg', prefix='gnomecast_pid%i_thumbnail_' % os.getpid())[1]
    ...
    def load_subtitles(self):
        ...
        for stream in self.subtitles:
            srt_fn = tempfile.mkstemp(
                suffix='.srt', prefix='gnomecast_pid%i_subtitles_' % os.getpid())[1]
            files.append(srt_fn)
    ...
    def details(self):
        fields = [
            ...
            'Video: %s' % ', '.join(
                ['%s (%s)' % (s.title, s.codec) for s in self.video_streams]),
...
class Transcoder(object):
    ...
    def __init__(self, cast, fmd, video_stream, audio_stream, done_callback, error_callback, prev_transcoder=None, force_audio=False, force_video=False, fake=False):
        ...
        print('transcode, transcode_video, transcode_audio',
        ...
        if self.transcode:
            ...
            device_info = HARDWARE.get(
                (self.cast.device.manufacturer, self.cast.device.model_name))
            ac3 = device_info.ac3 if device_info else None
            transcode_audio_to = 'ac3' if (
                ac3 or ac3 is None) and audio_stream and audio_stream.channels > 2 else 'mp3'

            self.transcode_cmd = [
                'ffmpeg', '-i', self.source_fn, '-map', self.video_stream.index]
            ...
            # '-movflags', 'faststart'
            self.transcode_cmd += ['-c:v',
                                   'h264' if self.transcode_video else 'copy']
            self.transcode_cmd += [self.trans_fn]
            print(
                ' '.join(["'%s'" % s if ' ' in s else s for s in self.transcode_cmd]))
            if fake:
                self.p = None
                self.monitor()
            else:
    ...
    def can_play_video_codec(self, video_codec):
        h265 = True
        if self.cast.device.cast_type == 'audio':
            h265 = False
        device_info = HARDWARE.get(
            (self.cast.device.manufacturer, self.cast.device.model_name))
    ...
    def wait_for_byte(self, offset, buffer=128 * 1024 * 1024):
        ...
        if self.source_fn.lower().split(".")[-1] == 'mp4':
            while offset > self.progress_bytes + buffer:
                print('waiting for', offset, 'at',
                      self.progress_bytes + buffer)
                time.sleep(2)
        else:
            while not self.done:
        ...
        while self.p:
            ...
            if byte != b'':
                ...
                if byte == b'\r':
                    ...
                    self.progress_seconds = parse_ffmpeg_time(
                        d.get('time', '00:00:00'))
                    line = b''
        if self.p:
            self.p.stdout.close()
...
class Gnomecast(object):
        ...
        if not ffmpeg_available:
            def f():
                dialog = Gtk.MessageDialog(
                    self.win, 0, Gtk.MessageType.ERROR, Gtk.ButtonsType.CLOSE, "FFMPEG not Found")
    ...
    def start_server(self):
        ...
        def video(id, ext):
            ...
            ranges = list(bottle.parse_range_header(
        ...
        httpserver.serve(handler, host=self.ip, port=str(
            self.port), daemon_threads=True)
...
#      notes.append('Cast: %s' % self.last_known_player_state)
    ...
    def init_casts(self, widget=None, device=None):
        ...
        self.cast_store.append(
            [None, "Searching local network - please wait..."])
        self.cast_combo.set_active(0)
        threading.Thread(target=self.load_casts, kwargs={
                         'device': device}).start()
        ...
        def f():
            ...
            if device:
                ...
                if not found:
                    self.cast_combo.set_active(0)
                    dialog = Gtk.MessageDialog(
                        self.win, 0, Gtk.MessageType.ERROR, Gtk.ButtonsType.CLOSE, "Chromecast Not Found")
                    dialog.format_secondary_text(
    ...
    def update_media_button_states(self):
        ...
        self.stop_button.set_sensitive(bool(
            self.transcoder and self.cast and mc.status.player_state in ('BUFFERING', 'PLAYING', 'PAUSED')))
        self.rewind_button.set_sensitive(bool(
            self.transcoder and self.cast and mc.status.player_state in ('BUFFERING', 'PLAYING', 'PAUSED')))
        self.forward_button.set_sensitive(bool(
            self.transcoder and self.cast and mc.status.player_state in ('BUFFERING', 'PLAYING', 'PAUSED')))
    ...
    def build_gui(self):
        ...
        refresh_button = Gtk.Button(
            None, image=Gtk.Image(stock=Gtk.STOCK_REFRESH))
        ...
        self.files_view.append_column(
            Gtk.TreeViewColumn("Duration", r, text=3))
        ...
        self.files_view.connect(
            "row-activated", self.on_files_view_row_activated)
        ...
        self.file_button = Gtk.Button(
            None, image=Gtk.Image(stock=Gtk.STOCK_ADD))
        self.file_button.set_tooltip_text(
            'Add one or more audio or video files...')
        ...
        self.remove_button = Gtk.Button(
            None, image=Gtk.Image(stock=Gtk.STOCK_REMOVE))
        ...
        self.subtitle_store = Gtk.ListStore(
            str, object, object)  # title, stream, callback
    ...
    def seek_delta(self, delta):
        seconds = self.cast.media_controller.status.current_time + \
            time.time() - self.last_time_current_time + delta
        self.last_time_current_time = time.time()
    ...
    def play_clicked(self, widget):
        ...
        print('mc.status.player_state',
              mc.status.player_state, self.fn, hash(self.fn))
        if mc.status.player_state in ('IDLE', 'UNKNOWN') or self.last_fn_played != self.fn:
            ...
            mc.play_media('http://%s:%s/media/%s.%s' % (self.ip, self.port, hash(self.fn),
                                                        ext), 'audio/%s' % ext if ext in AUDIO_EXTS else 'video/mp4', **kwargs)
            print(cast.status)
            print(mc.status)
            self.prep_next_transcode()
    ...
    def select_subtitles_file(self, fn):
        if not os.path.isfile(fn):
            def f():
                dialog = Gtk.MessageDialog(
                    self.win, 0, Gtk.MessageType.ERROR, Gtk.ButtonsType.CLOSE, "File Not Found")
                dialog.format_secondary_text(
                    "Could not find subtitles file: %s" % fn)
                dialog.run()
                dialog.destroy()
            GLib.idle_add(f)
            return
        ...
        if not os.path.isfile(fn):
            def f():
                dialog = Gtk.MessageDialog(
                    self.win, 0, Gtk.MessageType.ERROR, Gtk.ButtonsType.CLOSE, "File Not Found")
                dialog.format_secondary_text(
                    "Could not find media file: %s" % fn)
                dialog.run()
                dialog.destroy()
            GLib.idle_add(f)
            return
    ...
    def prep_next_transcode(self):
        ...
        for row in self.files_store:
            ...
            if transcode_next and not transcoder:
                ...
                transcoder = Transcoder(self.cast, fmd, fmd.audio_streams[0] if fmd.audio_streams else None, lambda did_transcode=None: GLib.idle_add(
                    self.update_status, did_transcode), self.error_callback, transcoder)
                row[7] = transcoder
                transcode_next = False
    ...
    def select_cast(self, cast):
        ...
        if cast:
            ...
            self.volume_button.set_value(
                cast.media_controller.status.volume_level)
        self.last_known_player_state = None
        self.update_media_button_states()
        threading.Thread(target=self.update_transcoders).start()
    ...
    def show_file_info(self, b=None):
        ...
        msg = '\n' + fmd.details()
        if self.cast:
            msg += '\nDevice: %s (%s)' % (self.cast.device.model_name,
                                          self.cast.device.manufacturer)
...
```%s```''' % (msg, fmd, fmd._important_ffmpeg)
            ...
            dialogWindow.add_action_widget(Gtk.LinkButton(
                url, label="Report File Doesn't Play"), 10)
        ...
        if (response == Gtk.ResponseType.OK) and (text != ''):
            ...
            try:
                ...
                dialog = Gtk.MessageDialog(
                    self.win, 0, Gtk.MessageType.ERROR, Gtk.ButtonsType.CLOSE, "Chromecast Not Found")
                dialog.format_secondary_text(
                    "The Chromecast '%s' wasn't found." % text)
                dialog.run()
                dialog.destroy()
...
def main():
    ...
    caster = Gnomecast()
    arg_parse(sys.argv[1:], {'s': 'subtitles',
                             'd': 'device'}, caster.run, USAGE)
